...
function openMovieDetails(selectedMovie) {
  ...
  document.querySelector('#details-genre').textContent = setFilmGenres(
    selectedMovie,
  );
  ...
  fetchMovies(selectedMovie.id).then(res => {
    if (res === null) {
      document
        .getElementById('js-movieTrailer')
        .insertAdjacentHTML(
          'beforeend',
          '<div style="background-color: tomato; color: teal; height: 200px;font-size: 20px; padding: 10px"> Короче, немає тут ніякого трейлера</div>',
        );
      return;
    }
...
function setFilmGenres(movie) {
  ...
  if (movie.genre_ids) {
    ...
    filmGeneres = movie.genres.reduce(
